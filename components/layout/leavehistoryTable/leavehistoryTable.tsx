...
const LeaveHistoryTable = () => {
  ...
  const { data: leaveData, error, isLoading } = useSWR<LeaveRequest[]>('/api/leave-requests', fetcher);
  ...
  return (
    ...
    <tbody>
    {leaveData && leaveData.map((request) => (
      <tr key={request.id}>
      <td>{request.datesRequested}</td>
      <td>{request.type}</td>
      <td>{request.mode}</td>
      <td>{request.days}</td>
      ...
      <button onClick={() => toggleMenu(request.id)} className={styles.menuButton}>⋮</button>
